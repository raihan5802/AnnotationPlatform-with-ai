...
const ShapeRenderer = ({
  ...
  const getDeletePosition = (annotation) => {
  ...
  // Handle vertex drag
  const handleVertexDragStart = (annIndex, vertexIndex, e) => {
    e.cancelBubble = true;
  ...
  const handlePolyDragEnd = (index, e) => {
    const { x, y } = e.target.position();
  ...
  const handleRemoveVertex = (annIndex, vertexIndex, e) => {
    e.preventDefault();
    e.cancelBubble = true;
  ...
  const handleInsertVertex = (annIndex, vertexIndex, e) => {
    e.cancelBubble = true;
  ...
  const vertexCursorEnter = (annIndex, vertexIndex) => {
  ...
  const isNearPoint = (x, y, point, threshold = 10) => {
    const dx = point.x - x;
    const dy = point.y - y;
    return Math.sqrt(dx * dx + dy * dy) <= threshold / scale;
  };

  const handlePolyAreaContextMenu = (annIndex, e) => {
