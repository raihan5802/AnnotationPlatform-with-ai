...
app.post('/api/projects', (req, res) => {
  try {
    const { userId, projectName, folderId, projectType, labelClasses, dataProviderEmail, collaboratorEmail, hasData = true } = req.body;
    ...
    // Add data provider role if provided
    if (dataProviderEmail) {
      // Find data provider user ID from email
      const usersFilePath = path.join(__dirname, 'users.csv');
      if (fs.existsSync(usersFilePath)) {
        const usersContent = fs.readFileSync(usersFilePath, 'utf8');
        const userLines = usersContent.trim().split('\n').slice(1);
        ...
        const dataProviderLine = userLines.find(line => {
          ...
          return email === dataProviderEmail;
        });
    ...
    // Add collaborator role if provided
    if (collaboratorEmail) {
      // Find collaborator user ID from email
      const usersFilePath = path.join(__dirname, 'users.csv');
      if (fs.existsSync(usersFilePath)) {
        const usersContent = fs.readFileSync(usersFilePath, 'utf8');
        const userLines = usersContent.trim().split('\n').slice(1);
        ...
        const collaboratorLine = userLines.find(line => {
          const [id, username, email] = line.split(',');
          return email === collaboratorEmail;
        });
